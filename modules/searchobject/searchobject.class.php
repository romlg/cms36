...
class TSearchObject extends TObject {
	...
	function show(&$params) {
		...
		$tpl->assign($res);
		$ret['search_menu'] = $tpl->fetch("search_menu.html");
		...
		if (isset($res['price_type']) && $res['price_type'] > 0) {
			...
			if ($res['price_type']==1){
				if (isset($res['price_from']) && (int)$res['price_from'] > 0) {
					...
					if ($numeric_r) $price_from = $price_from - $numeric_r;
					if ($percent) $price_from = $price_from - round($price_from*$percent/100);
					$sql .= " AND (o.price_rub>=".$price_from.")";
				}
				if (isset($res['price_to']) && (int)$res['price_to'] > 0) {
					$price_to = (int)str_replace(" ", "", $res['price_to']);
					if ($numeric_r) $price_to = $price_to - $numeric_r;
					if ($percent) $price_to = $price_to - round($price_to*$percent/100);
			...
			if ($res['price_type']==2){
				if (isset($res['price_from']) && (int)$res['price_from'] > 0) {
					...
					if ($numeric_r) $price_from = $price_from - $numeric_s;
					if ($percent) $price_from = $price_from - round($price_from*$percent/100);
					$sql .= " AND (o.price_dollar>=".$price_from.")";
				}
				if (isset($res['price_to']) && (int)$res['price_to'] > 0) {
					$price_to = (int)$res['price_to'];
					if ($numeric_r) $price_to = $price_to - $numeric_s;
					if ($percent) $price_to = $price_to - round($price_to*$percent/100);
	...
	function formatObjList ($data, $price_type = '1', $count, $page_id, $limit, $sql, $favoriteFlag = false) {
		...
		foreach ($data AS $key=>$value) {
			...
			if ($price_type==1){
				...
				if ($percent) $price = $price + round(($price*$percent)/100);
				if ($numeric_r) $price = $price + $numeric_r;
				...
				$price = (int) str_replace(" ", "", $price);
				if ($percent) $price = $price + round(($price*$percent)/100);
				if ($numeric_s) $price = $price + $numeric_s;
				$price = number_format($price, ' ', ' ', ' ');
				$price_field = 'price_dollar';
			}
/*
			$event = "onmouseout=\"GEvent.trigger(marker[".$i."],'mouseout')\"
			onmouseover=\"GEvent.trigger(marker[".$i."],'mouseover'); q2000.search.showMap(".$i."); return false; \"";
*/
			$event = "";
			...
			$data[$key]['favorile']		= (in_array ($value['id'], $obj_favorite)) ? '<img src="/images/ico_added_favorite.png" onClick="q2000.search.addToFavoriteObject(\'\','.$value['id'].'); return false;" alt="" />' : '<img src="/images/ico_add_cart.png" onClick="q2000.search.addToFavoriteObject(\'add\','.$value['id'].'); return false;" alt="" />';
		...
		$variable = sql_getRows('SELECT MIN('.$price_field.') AS minimum, MAX('.$price_field.') AS maximum FROM `objects` AS o WHERE 1 AND o.visible > 0 '.$sql);
		...
		if ($cnt) {
			$navArr = array();
			if($page_id != 0) $navArr[] = " <a class=\"prev\" href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id - 1)."'; q2000.search.load(".$loadFlag.");\">Предыдущая</a>\n";
			else		  $navArr[] = " <a class=\"prevDisabled\" href='javascript:void(0);'\" title=\"предыдущая\">Предыдущая</a>\n";
			if($page_id-6>=0) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '0'; q2000.search.load(".$loadFlag.");\">1</a>\n";

			// Находим две ближайшие станицы с обоих краев, если они есть
			if($page_id-6>0) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id-6)."'; q2000.search.load(".$loadFlag.");\">...</a>\n";
			if($page_id-5>=0) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id-5)."'; q2000.search.load(".$loadFlag.");\">".($page_id-4)."</a>\n";
			if($page_id-4>=0) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id-4)."'; q2000.search.load(".$loadFlag.");\">".($page_id-3)."</a>\n";
			if($page_id-3>=0) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id-3)."'; q2000.search.load(".$loadFlag.");\">".($page_id-2)."</a>\n";
			if($page_id-2>=0) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id-2)."'; q2000.search.load(".$loadFlag.");\">".($page_id-1)."</a>\n";
			if($page_id-1>=0) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id-1)."'; q2000.search.load(".$loadFlag.");\">".($page_id-0)."</a>\n";

			$navArr[] = "<span>".($page_id+1)."</span>\n";

			if($page_id+1<=$cnt) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id+1)."'; q2000.search.load(".$loadFlag.");\">".($page_id+2)."</a>\n";
			if($page_id+2<=$cnt) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id+2)."'; q2000.search.load(".$loadFlag.");\">".($page_id+3)."</a>\n";
			if($page_id+3<=$cnt) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id+3)."'; q2000.search.load(".$loadFlag.");\">".($page_id+4)."</a>\n";
			if($page_id+4<=$cnt) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id+4)."'; q2000.search.load(".$loadFlag.");\">".($page_id+5)."</a>\n";
			if($page_id+5<=$cnt) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id+5)."'; q2000.search.load(".$loadFlag.");\">".($page_id+6)."</a>\n";
			if($page_id+6<$cnt) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id+6)."'; q2000.search.load(".$loadFlag.");\">...</a>\n";

			if($page_id+6<=$cnt) $navArr[] = " <a href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($cnt)."'; q2000.search.load(".$loadFlag.");\">".($cnt+1)."</a>\n";
			if($page_id != $cnt) $navArr[] = " <a class=\"next\" href='javascript:void(0);' onClick=\"document.getElementById('page_obj').value = '".($page_id + 1)."'; q2000.search.load(".$loadFlag.");\">Следующая</a>\n";
	...
	function searchResult(){
		...
		$table .= "<tr>\n
				<th><a class=\"sort\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('img'); return false;\">Комн.</th> \n
				<th><a class=\"sort\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('room'); return false;\">Карта</a></th>\n
				<th><a class=\"sort top\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('metro'); return false;\">Метро</a></th>\n
				<th><a class=\"sort bottom\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('address'); return false;\">Адрес</a></th>\n
				<th><a class=\"sort\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('storey'); return false;\">Этаж</a></th>\n
				<th><a class=\"sort\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('area'); return false;\">Площадь</a></th>\n
				<th><a class=\"sort\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('price'); return false;\">Цена ".(($fld['price_type']==1)?" руб.":" $")."</a></th>\n
				<th><a class=\"sort\" href='javascript:void(0);' onClick=\"q2000.search.tableSort('stars'); return false;\">Комфортность дома</a> </th>\n
				<th>Контакты</th>\n
				<th></th>\n
			</tr>\n";
       if ($list){
		...
		foreach ($list['list'] AS $key=>$value) {
			$table .= "<tr ".$value['class']." ".$value['event']." id='".$value['tr_id']."'>\n
				...
				<td>\n
					...
					<img id=\"".$i."\" src=\"/images/map_ico.png\" onmouseover=\"q2000.search.showMapGoogle(".$i."); return false;\"  onclick=\"this.onmouseout=null; q2000.search.showMapGoogle(".$i."); return false;\" onmouseout=\"q2000.search.closeDiv('object_mapmouse'); return false;\" style=\"cursor:pointer\"  title=\"Нажмите чтобы закрепить карту\"  alt=\"\" />\n
				</td>\n
				<td>".$value['metroname']."</td>
				<td><a href=\"".$value['href']."/object/".$value['id']." \"  target=\"_blank\">".$value['address']."</a></td>
		...
		if ($favoriteFlag) $data['favoriteLink'] = iconv('windows-1251', 'utf-8', "<a href=\"javascript:void(0);\" onClick=\"q2000.search.addToFavorite('delete');\">Удалить выделенные объекты из избранного</a>");

		if ($favoriteFlag) {
			$data['show_f'] = iconv('windows-1251', 'utf-8', "<a href=\"javascript:void(0);\" onClick=\"q2000.search.load(); return false;\"> <img src=\"/images/back.png\"> Вернуться к результатам поиска</a>");
		} else {
			if (isset($_COOKIE['obj_favorite'])){
				$obj_favorite = unserialize($_COOKIE['obj_favorite']);
				if (!empty($obj_favorite)) {
					$data['show_f'] = iconv('windows-1251', 'utf-8', " <a href=\"javascript:void(0);\" onClick=\"q2000.search.load(true); return false;\"> <img src=\"/images/favorite.png\"> Избранное(".count($obj_favorite).")</a>");
				}
			} else {
				$data['show_f'] = iconv('windows-1251', 'utf-8', "<a href=\"javascript:void(0);\" onClick=\"q2000.search.load(true); return false;\"></a>");
			}
		}
	...
	function searchAddFavorite(){
		...
		if ($id) {
			if ($doit=="add") {
				...
				$favorite_img = '<img src="/images/ico_added_favorite.png" onClick="q2000.search.addToFavoriteObject(\'\','.$id.'); return false;" alt="" />';
			} else {
				$arr = "";
				if (isset($_COOKIE['obj_favorite'])){
					$obj_favorite = unserialize($_COOKIE['obj_favorite']);
				...
				$favorite_img = '<img src="/images/ico_add_cart.png" onClick="q2000.search.addToFavoriteObject(\'add\','.$id.'); return false;" alt="" />';
			}

			$favorite_count = (count($fld)) ? "  <a href=\"javascript:void(0);\" onClick=\"q2000.search.load(true); return false;\"> <img src=\"/images/favorite.png\"> Избранное (".count($fld).")</a>" : "";
		} else {
			$favorite_count	= "";
		}
	...
	function sendOrder(){
		...
		$data = array(
			'msg_form'=>iconv('windows-1251', 'utf-8', "Спасибо, Ваша заявка отправлена. В ближайшее  время наши менеджеры свяжутся с Вами."),
		);
		header('Content-Type: application/json; charset=utf-8');
		echo json_encode($data);
		exit;
