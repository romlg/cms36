...
class TObject {
    ...
    function getPathToObject($id, $obj_type_id = '') {
        ...
        if (isset($this->__pids[$sql])) $pid = $this->__pids[$sql];
            ...
            $pid = sql_getValue('SELECT pid FROM '.$this->elem_table.' WHERE 1 '.$sql);
            $this->__pids[$sql] = $pid;
        }
    ...
    function Show(&$params){
        ...
        $pos = array_search('object', $pids); // Позиция слова object в пути страницы
...
*/
		...
		$images = sql_getRows('SELECT pid, smallimagepath FROM obj_elem_images WHERE visible > 0 GROUP BY pid ORDER BY pid, priority', true);
		...
		$photo_counts = sql_getRows('SELECT pid, COUNT(*) FROM obj_elem_images WHERE type="photo" AND visible > 0 GROUP BY pid', true);
    ...
    function getOneObject($id, $pids, $pos, $tab) {
        ...
        $page->tpl->assign(array('info' => $info, 'real_path' => $url, 'current_tab' => $tab, 'fields' => $this->inside_fields, 'type' => 'object'));
    ...
    function getInfo($id, $tab, $_id) {
        global $settings;
        $page = &Registry::get('TPage');
		$info = sql_getRow('SELECT * FROM objects WHERE id='.$id);
		...
		$info['photos'] = sql_getRows('SELECT * FROM obj_elem_images WHERE pid='.$id.' AND type="photo" AND visible > 0');
		$info['count_flash'] = sql_getValue('SELECT COUNT(*) FROM obj_elem_images WHERE pid='.$id.' AND type="flash" AND visible > 0');
    ...
    function showPhoto(&$info) {
    	$rows = sql_getRows('SELECT * FROM obj_elem_images WHERE type="photo" AND pid='.$info['id'].' AND visible > 0 ORDER BY priority');
    ...
    function showFlash(&$info, $_id) {
        $info['gallery'] = sql_getRows('SELECT * FROM obj_elem_images WHERE type="flash" AND pid='.$info['id'].' AND visible > 0 ORDER BY priority');
       	if (!$_id) {
       		// Берем первое изображение
       		$_id = $info['gallery'][0]['id'];
       	}
       	$flash_file = sql_getValue('SELECT imagepath FROM obj_elem_images WHERE id='.$_id);
