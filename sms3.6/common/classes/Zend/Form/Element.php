    ...
    public function getValue()
    {
        ...
        if ($_POST) {
            if (is_array($valueFiltered)){
                $valueFiltered = $this->recursionArrayIconv(&$valueFiltered);
            } else {
                $valueFiltered = iconv('windows-1251', 'utf-8', $valueFiltered);
            }
        }
        return $valueFiltered;
    }

    function recursionArrayIconv($array)
    {
        if(is_array($array)) {
            for($i=0; $i<count($array); $i++) {
                if(is_array($array[$i])) {
                    recursionArrayIconv(&$array[$i]);
                } else {
