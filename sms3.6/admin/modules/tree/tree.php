...
class TTree extends TTable
{
    ...
    function Show() {
        ...
        if (empty($columns)) {
            ...
            sql_query("UPDATE `tree` SET `is_link` = IF(`redirect`<>'',1,0)");
        }
    ...
    function getOneItem($id, $row, $root_id, $hidden_url = '', $params) {
        ...
        $item['note'] = $note;
        if ($params['treeurl']) {
            $item['java_href'] = "javascript:selecturl(" . $row['id'] . ", \"" . addslashes(str_replace(array('"', '&quot;'), '', $row['name'])) . "\", \"" . $row['dir'] . "\")";
        }
        ...
        // пересчет приоритетов вложенных разделов
        $cnt_priority = sql_getRow("SELECT COUNT(DISTINCT priority) as cnt, COUNT(1) as all_cnt FROM tree WHERE pid={$row['id']} AND id<>pid AND visible>-1");
        if ($cnt_priority['cnt'] != $cnt_priority['all_cnt']) {
            $subitems = sql_getColumn("SELECT id FROM tree WHERE pid={$row['id']} AND id<>pid AND visible>-1 ORDER BY priority, name");
            if ($subitems) {
                $priority = 1;
                foreach ($subitems as $subitem_id) {
                    sql_query("UPDATE tree SET priority={$priority} WHERE id={$subitem_id}");
                    $priority++;
                }
            }
        }
        ...
        if (!$id) {
            ...
            if ($row) {
                ...
                if (count($site_domains)>1) {
                    $seeurl = 'http://' . getSiteByRootID($row['root_id']) . '/' . getLangByRootID($row['root_id']) . $row['dir'] . $path_id;
                } else {
                    $sitelng = getLangByRootID($row['root_id']);
                    $seeurl = (LANG_DEFAULT==$sitelng) ? $row['dir'] . $path_id : '/' . $sitelng . $row['dir'] . $path_id;
                }
                $html .= '&nbsp;<a href="'.$seeurl.'" target="_blank"><img src="/admin/images/tree/ico_see.gif" alt="Просмотр"></a>';
    ...
    function showTreeLinksHtml() {
        ...
        if (!$ids) {
            ...
            foreach ($ids as $id) {
                ...
                if (count($site_domains)>1) {
                    $seeurl = 'http://' . getSiteByRootID(@$rows[$id]['root_id']) . '/' . getLangByRootID(@$rows[$id]['root_id']) . $rows[$id]['dir'] . $path_id;
                } else {
                    $sitelng = getLangByRootID(@$rows[$id]['root_id']);
                    $seeurl = (LANG_DEFAULT==$sitelng) ? @$rows[$id]['dir'] . $path_id : '/' . $sitelng . @$rows[$id]['dir'] . $path_id;
                }
                $html .= '&nbsp;<a href="'.$seeurl.'" target="_blank"><img src="/admin/images/tree/ico_see.gif" alt="Просмотр"></a>';
                $html .= '</div>';
            }
