...
class TObjectEditor extends TTable
{
    ...
    function editForm() {
        ...
        foreach ($this->tabs AS $key => $value) {
            ...
            $data['form_content'][$key]['name'] = $value['display'][int_lang()];
    ...
    function save($id, $fld, $close_wind=false) {
        ...
        foreach ($tabs as $k => $v) {
            ...
            $this->elem_object[$k]->ElemRedactAfter($fld[$k], $pid);
            ...
            if ($pid && in_array($v['conf']['elem_type'], array('multi', 'link'))) {
                ...
                $this->EnumPriority(
                    ...
                    $id_field,
                    isset($elem_object->elem_fields['priority_field']) ? $elem_object->elem_fields['priority_field'] : 'priority'
                );
            }
        ...
        if ($error_tab) {
			...
			<script>window.parent.openTab('$error_tab');</script>".
            $this->fancyAlert(addslashes($error));
        }
        ...
        //определим откуда сохранение - обычная форма ил иво всплывающем фрейме
        $frame = get('frame', '', 'pg');
        if ($frame == 'view') {
        ...
        if ((int)get('id', 0, 'g') && $close_wind) {
            return $this->fancyAlert('Изменения сохранены');
        } else {
    ...
    function afterSaveInFrame($id, $pid) {
        ...
        $parent = $pid ? (int)sql_getValue("SELECT " . $elem_object->elem_fields['id_field'] . " FROM " . $table . " WHERE id=" . $pid) : 0;
    ...
    function fancyAlert ($text, $id) {
        $id = $id ? $id : "resultMsg";
        $code = "$('#".$id."', top.document).html('<h2>".$text."</h2>');";
        $code .= "window.parent.openAlert();";
        return "<script type='text/javascript' src='./js/jquery-1.5.2.min.js'></script><script type='text/javascript'>".$code."</script>";
    }
}