...
function TBToolbar_GetHTML(where)
{
  if (where == 'down') { this.Bands = this.downBands;}
  else { this.Bands = this.upBands;}
...
function TBToolbar_LoadButtonsSet(toolbarSetName)
{
	...
	if (config.downToolbarSets[toolbarSetName]){
		var downToolbarSet = config.downToolbarSets[toolbarSetName] ;
		this.downBands = new TBBandList() ;
		var oBand ;
		var sItem ;

		for (iBand = 0 ; iBand < downToolbarSet.length ; iBand++)
		{
			oBand = this.downBands.Add() ;
			for (iItem in downToolbarSet[iBand])
			{
				sItem = downToolbarSet[iBand][iItem] ;
				if ((sItem) == '-')
					oBand.Items.Add(new TBSeparator()) ;
				else
					oBand.Items.Add(oTB_Items[sItem]) ;
