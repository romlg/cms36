...
function changeAlign(align) {
	oSel = objContent.DOM.selection.createRange().item(0);
	oSel.align = align;
	oSel.CLASS = align;
}
...
function dialogLink()
{
	...
	{
		FCKShowDialog("dialog/fck_link.html"+showhome, window, 400, 190);
		SetFocus() ;
	}
}
...
//Добавление полного удаления формата
var htmlText;
function cleanW( htmlText )
{
	   if (htmlText == '') {
			objContent.DOM.body.innerText = objContent.DOM.body.innerText;
	   }

		/*
		// Remove all tags
		htmlText = htmlText.replace(/<\/*[A-Za-z]*>/gi, "" ); */
		// Remove Class attributes
		htmlText = htmlText.replace(/<(\w[^>]*) class=([^ |>]*)([^>]*)/gi, "<$1$3") ;
		// Remove Style attributes
		htmlText = htmlText.replace(/<(\w[^>]*) style="([^"]*)"([^>]*)/gi, "<$1$3") ;
		// Remove Lang attributes
		htmlText = htmlText.replace(/<(\w[^>]*) lang=([^ |>]*)([^>]*)/gi, "<$1$3") ;
		// Remove XML elements and declarations
	   ...
	   // Remove all SPAN tags
		htmlText = htmlText.replace(/<\/?SPAN[^>]*>/gi, "" );
       // Remove all DIV tags
		htmlText = htmlText.replace(/<\/?DIV[^>]*>/gi, "" );	
		// Remove all p tags
		htmlText = htmlText.replace(/<\/?P[^>]*>/gi, "" );
		// Remove all em tags
		htmlText = htmlText.replace(/<\/?em[^>]*>/gi, "" );
		// Remove all b tags
		htmlText = htmlText.replace(/<\/?b[^>]*>/gi, "" );
		// Remove all u tags
		htmlText = htmlText.replace(/<\/?u[^>]*>/gi, "" );
		// Remove all strike tags
		htmlText = htmlText.replace(/<\/?strike[^>]*>/gi, "" );
		// Remove all strong tags
		htmlText = htmlText.replace(/<\/?strong[^>]*>/gi, "" );


       // Remove all font tags
		htmlText = htmlText.replace(/<\/?font[^>]*>/gi, "" );
		// Transform <P> to <DIV>
        //var re = new RegExp("(<P)([^>]*>.*?)(<\/P>)","gi") ;    // Different because of a IE 5.0 error
		//html = html.replace( re, "<div$2</div>" ) ;
	insertHtml( htmlText ) ;
...
function versions()
{
	var html = FCKShowDialog("../../dialog.php?page=versions&table="+window.top.frames['cnt'].TABLE+"&id="+window.top.frames['cnt'].ID+"&instance="+oLinkedField.name, window, 460, 290);
	if (html) insertHtml(html) ;
	SetFocus() ;
}