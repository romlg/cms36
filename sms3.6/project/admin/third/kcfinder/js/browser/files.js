        ...
        $.each(browser.files, function(i, file) {
            ...
            if (_.kuki.get('view') == 'list') {
                ...
                if (file.thumb)
                    ...
                    var icon = '';
                    if (browser.uploadURL != '/') icon += browser.uploadURL;
                    icon += '/' + browser.dir + '/' + file.name;
...
browser.returnFile = function(file) {
    ...
    var fileURL = "";
    if (file.substr) fileURL = file;
    else {
        if (browser.uploadURL != "/") fileURL += browser.uploadURL;
        fileURL += '/' + browser.dir + '/' + file.data('name');
    }
...
browser.returnFiles = function(files) {
    if (this.opener.callBackMultiple && files.length) {
        ...
        $.each(files, function(i, file) {
            rfiles[i] = "";
            if (browser.uploadURL != "/") rfiles[i] += browser.uploadURL;
...
browser.menuFile = function(file, e) {
    ...
    $('.menu a[href="kcact:view"]').click(function() {
        ...
        var showImage = function(data) {
            url = "";
            if (browser.uploadURL != "/") url = browser.uploadURL;
            url += '/' + browser.dir + '/' + data.name;
            url = _.escapeDirs(url) + '?ts=' + ts,
