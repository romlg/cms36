...
class TTreeUtils {
	...
	function GetSubItems($id, $types = 'all', $show_invisible = false) {
        ...
        $sql = $this->Select('*', $item_elems, $pid_where.' AND '.$id_where.($show_invisible ? '' : ' AND '.$this->table.'.visible>0').$types_where.' GROUP BY '.$this->table.'.id ORDER BY '.$this->table.'.pid, '.$this->table.'.priority, '.(defined('LANG_SELECT') && LANG_SELECT ? $this->table.'.name_'.lang() : $this->table.'.name'));
