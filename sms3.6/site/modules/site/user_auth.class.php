...
class TUserAuth {
	...
	function setLoginCookie() {
		...
		if ($_SERVER['REMOTE_ADDR'] == '127.0.0.1') $domain = '';
		else {
		$domain = $_SERVER['HTTP_HOST'];
		if ( strtolower( substr($domain, 0, 4) ) == 'www.' ) $domain = substr($domain, 4); 
    		if ( substr($domain, 0, 1) != '.' && strpos($domain, '.') !== false) $domain = '.'.$domain; 
		}		
		
		setcookie(AUTH_HASH_COOKIE_NAME, $t_str, time() + 24 * 3600 * 30, '/', $domain);
		setcookie(AUTH_USER_COOKIE_NAME, $this->userdata['id'], time() + 24 * 3600 * 30, '/', $domain);
	}
	...
	function fp_getUserEmail() {
		...
		if (isset($this->cached_user['email'])){
			return $this->cached_user['email'];
		}
