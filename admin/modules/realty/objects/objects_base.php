...
class TObjects_base extends TTable {
	...
	function TObjects_base() {
		...
		$str[get_class_name($this)] = array_merge($str[get_class_name($this)], array(
			...
			'sell_type'		=> array('Предназначен для',		'Sell_type',),
	...
	function Show_base($columns, $dataset = array(), $where = '', $function = '') {
		...
		if (!empty($dataset))
			...
			'from' => $this->table.' AS o
					...
					LEFT JOIN obj_managers om             ON ( o.manager_id = om.id )
    ...
    function editgetCity(){
		...
		$id = (int)get('id', 0, 'g');
		$ret = array();
		if ($region) $ret = sql_getRows('SELECT id, name FROM obj_locat_districts WHERE pid='.$region.' ORDER BY name', true);
		
		$selected = sql_getValue('SELECT city FROM objects WHERE id='.$id);
	...
	//-----------------------------------------------------------------------
    function editgetClass(){

		$id = (int)get('id', 0, 'g');
		$table = get('table', '', 'g');
		$belong = (int)get('belong', 0, 'g');
		$childs = $this->getChilds('obj_class', 'ORDER BY priority', null, 'belong='.$belong);
		$this->getList($childs, $ret);

		if ($table == 'elem_searchtemplate') $selected = sql_getValue('SELECT obj_class FROM '.$table.' WHERE id='.$id);
		else $selected = sql_getValue('SELECT obj_class_id FROM '.$table.' WHERE id='.$id);

		header('Content-Type: text/xml');
		$str = '<?xml version="1.0" encoding="windows-1251" standalone="yes" ?><body>';
		$str .= '<item><id>0</id><name>'.$this->str('all').'</name><selected>'.($selected == '0' ? '1' : '0').'</selected></item>';
		if ($ret) foreach ($ret as $k=>$v) $str .= '<item><id>'.$k.'</id><name>'.$v.'</name><selected>'.($k == $selected ? '1' : '0').'</selected></item>';
		echo $str;
		echo '</body>';
	}
    ...
    function editgetTypes(){
		...
		if ($table == 'elem_searchtemplate') $selected = sql_getValue('SELECT obj_type FROM '.$table.' WHERE id='.$id);
		else $selected = sql_getValue('SELECT obj_type_id FROM '.$table.' WHERE id='.$id);
