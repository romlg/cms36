...
class TObjects_Newbuild extends TTable {
	...
	function TObjects_Newbuild(){
		...
		$str[get_class_name($this)] = array_merge($str[get_class_name($this)], array(
			'title'			=> array('Новостройки',		'Newbuilds',),
			'room'			=> array('Кол-во комнат',			'Newbuild count'),
	...
	function Show() {
		...
		$ret['table'] = ajax_table(array('columns'	=> $columns,
			...
			'where'		=> 'obj_type_id="newbuild" AND o.visible > -1',
			'orderby'	=> 'o.priority',
			'click'		=> 'ID=cb.value',
	...
	function showprintForm(){
	    ...
	    $rows['objects'] = objects_getList(isset($_SESSION['RIEL']) ? $_SESSION['RIEL'] : array(), 'newbuild');

        objects_formatList($rows['objects']);

	    $rows['fields'] = objects_getFields(isset($_SESSION['RIEL']['print_filter']['newbuild_type']) ? $_SESSION['RIEL']['print_filter']['newbuild_type'] : 0, isset($_SESSION['RIEL']['print_filter']['o.region']) ? $_SESSION['RIEL']['print_filter']['o.region'] : 0);
    ...
    function ShowRecycle() {
        ...
        $row['table'] = ajax_table(array(
            ...
            'where'        => 'visible<0 and obj_type_id="newbuild"',
            'orderby'    => 'address',
...
$GLOBALS['objects_newbuild'] = &Registry::get('TObjects_Newbuild');
