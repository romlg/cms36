...
class TBrowsers extends TStat {

	var $name = 'stat/stat_browsers';
	...
	function GetTable() {
		...
		sql_query("
			CREATE TEMPORARY TABLE tmp_stat_browsers
			SELECT sess.agent_id AS agent_id, ag.name AS name
			FROM ".$this->sess_table." AS sess LEFT JOIN ".STAT_AGENTS_TABLE." AS ag ON ag.id=sess.agent_id
			WHERE sess.robot=0 $search_state");

		$count = sql_getValue("SELECT COUNT(DISTINCT(name)) FROM tmp_stat_browsers");
		...
		$data = sql_getRows("SELECT IF(name<>'',name,'Unknown') AS name, COUNT(*) AS kol, COUNT(*)/".$total_value."*100 AS proc FROM tmp_stat_browsers GROUP BY name ORDER BY kol DESC LIMIT ".$offset.", ".$limit);
        $total_head = array(
                '',
                $this->_str('visitors'),
        );
	...
	function GetCSVData() {
		...
		// заголовки
		echo $this->_str('browser').';'.$this->_str('visitors')."\n";

		$rows = sql_getRows("
			SELECT ag.name AS name, COUNT(*) as kol
			FROM ".$this->sess_table." AS sess LEFT JOIN ".STAT_AGENTS_TABLE." AS ag ON ag.id=sess.agent_id
...
$GLOBALS['stat__stat_browsers'] =  & Registry::get('TBrowsers');
