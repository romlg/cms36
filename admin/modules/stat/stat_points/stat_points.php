...
class TPoints extends TStat {

	var $name = 'stat/stat_points';
	...
	function GetTable() {
        ...
        sql_query("
			CREATE TEMPORARY TABLE tmp_stat_points
			SELECT pag.id AS page_id, pag.uri AS page, pag.host AS host
			FROM ".$this->sess_table." AS sess LEFT JOIN ".STAT_PAGES_TABLE." AS pag ON pag.id=sess.first_page
			WHERE sess.robot=0 AND sess.first_page!=0 $search_state");

		$count = sql_getValue("SELECT COUNT(DISTINCT(page_id)) FROM tmp_stat_points");
		$total_value = sql_getValue("SELECT COUNT(*) FROM ".$this->sess_table." AS sess LEFT JOIN ".STAT_PAGES_TABLE." AS pag ON pag.id=sess.first_page WHERE sess.robot=0 AND sess.first_page!=0");
		$data = sql_getRows("SELECT page, COUNT(*) AS kol, COUNT(*)/".$total_value."*100 AS proc, host, page_id FROM tmp_stat_points GROUP BY page_id ORDER BY kol DESC LIMIT ".$offset.", ".$limit);
        $total_head = array(
                '',
                $this->_str('visitors'),
        );
	...
	function GetCSVData() {
		...
		$rows = sql_getRows("
			...
			FROM ".$this->sess_table." AS sess LEFT JOIN ".STAT_PAGES_TABLE." AS pag ON pag.id=sess.first_page
			WHERE sess.robot=0 AND sess.first_page!=0 GROUP BY name ORDER BY kol DESC");
	...
	function GetDiagData() {
		...
		sql_query("
			...
			FROM ".$this->sess_table." AS sess LEFT JOIN ".STAT_PAGES_TABLE." AS pag ON pag.id=sess.first_page
			WHERE sess.robot=0 AND sess.first_page!=0");
	...
	function table_get_page($val, $row) {
		...
		return
			$this->table_get_advanced('points', $row['page_id']).
...
$GLOBALS['stat__stat_points'] =  & Registry::get('TPoints');
