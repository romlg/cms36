...
class TFormElement extends TElems
{
    ...
    function ElemForm() {
        $id = (int)get('id', 0);
        if (!$id) {
            return "Перед созданием формы требуется сохранить данные о разделе.";
        }

        $columns = sql_getRows("SHOW COLUMNS FROM `elem_form`", true);
        if (!isset($columns['submit_title'])) {
            sql_query("ALTER TABLE elem_form ADD submit_title VARCHAR( 100 ) NOT NULL COMMENT 'Надпись на кнопке сабмит';");
        }
    ...
    function ElemEdit($id, $row) {
        ...
        $id = sql_getValue("SELECT form_id FROM elem_form WHERE pid=" . $pid . " LIMIT 1"); // ID формы
