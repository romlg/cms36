...
if (typeof window.q2000 == 'undefined') {
    var q2000 = {};
}
q2000.search = {};

q2000.search.init = function(load) {
    var self = this;
    ...
    if (!load) {
		...
		$("#search-price").slider({
			...
			slide: function(event, ui) {
				$("#price-from").val(q2000.search.number_format(ui.values[0], 0, '.', ' '));
				$("#price-to").val(q2000.search.number_format(ui.values[1], 0, '.', ' '));
			},
			change: function(event, ui) {self.load()}
		});
		$("#price-from").val(q2000.search.number_format($("#search-price").slider("values", 0), 0, '.', ' '));
		$("#price-to").val(q2000.search.number_format($("#search-price").slider("values", 1), 0, '.', ' '));
 ...
 * Загрузка
 */
q2000.search.load = function(show_favorite) {
	var self = this;
    ...
    $.ajax({
        type: "POST",
		url: "search_result"+'?limit='+encodeURIComponent(limit)+'&sort='+encodeURIComponent(sort)+'&sort_d='+encodeURIComponent(sort_d)+furl,
        dataType: "json",
...
q2000.search._onSuccess = function(data) {
    var self = this;
    var p;
    var area = $('#loadArea2');
	...
	if (typeof data.favoriteLink != 'undefined') {
		...
		$('#favoriteLink').html("<a href=\"javascript:void(0);\" onClick=\"q2000.search.addToFavorite('add');\">Добавить выделенные объекты в избранное</a>");
	}
...
q2000.search.openDiv = function (id) {
    document.getElementById(id).style.display = 'block';
};

q2000.search.closeDiv = function (id) {
    document.getElementById(id).style.display = 'none';
};



q2000.search.addDistricts = function() {
    var self = this;
    var list = document.getElementById('raionList');
    list.innerHTML = '';
    if (raionArr.length > 0) {

        for (var i in raionArr) {
            var li = document.createElement('LI');
            li.innerHTML = "<a href=\"javascript:void(0);\" onClick=\"q2000.search.clearDistricts('i');\">"+raionArr[i]+"</a>";
            list.appendChild(li);
    ...
    if (cityArr.length > 0) {
        for (var i in cityArr) {
            var li = document.createElement('LI');
            li.innerHTML = cityArr[i];
            list.appendChild(li);
        }
    }
};

q2000.search.clearDistricts = function() {
    var self = this;
...
q2000.search.setMetroType = function(val) {
	var self = this;
...
q2000.search.clearMetroType = function() {
...
q2000.search.houseTypeAct = function(id) {
...
q2000.search.houseTypeGetList = function() {
...
q2000.search.rateAct = function(id) {
...
q2000.search.rateGetList = function() {
...
q2000.search.clearArea = function() {
...
q2000.search.clearSearch = function() {
	...
	q2000.search.clearRate(true);
	q2000.search.clearMetro();
	q2000.search.clearPrice();
	q2000.search.clearStorey();
	q2000.search.clearStoreysNumber();
	q2000.search.clearHouseType();
	q2000.search.clearArea();
	document.getElementById('ipoteka').checked=false;
	document.getElementById('credit').checked=false; q2000.search.load();
	$('#search_obj input[type=checkbox]').each(function() {this.checked=false;});
	this.load();
}

q2000.search.clearRate = function(home) {
    var self = this;
    for (var i in rateArr) {
...
q2000.search.tableSort = function(field) {
	var self = this;
...
q2000.search.selectAllToFavorite = function() {
	var obj;
	var chkd;
...
q2000.search.addToFavorite = function(doit) {
    var self = this;
	$.ajax({
		type: "POST",
...
q2000.search.addToFavoriteObject = function(doit, id) {
    var self = this;
	$.ajax({
		type: "POST",
		url: "search_add_to_favorite?id="+id+"&doit="+encodeURIComponent(doit),
...
q2000.search.number_format = function(number, decimals, dec_point, thousands_sep) {
...
q2000.search.checkObject = function (){
...
q2000.search.setRooms = function(clicked) {
...
q2000.search.addHouseTypes = function() {
    var self = this;
    var list = document.getElementById('house_list');
    list.innerHTML = '';
    if(houseArr.length > 0) {
...
q2000.search.showMap = function (ii) {
...
q2000.search.showMapGoogle = function (ii) {
	var lat, lng, address, name;
...
q2000.search.openTrQuery = function (){
...
$(document).ready(function() {
    ...
    if (href.search(/search_obj/) != -1) {
        q2000.search.init(true);
    } else {
        q2000.search.init(false);
    }
});